...
__constant__ uint32_t Const_In [16384]; //Maximum constant memory size: 65536
...
__global__ void s2g_gpu_gather_kernel(uint32_t *in, uint32_t *out, int len) {
  ...
  for (int inIdx = 0; inIdx < len; ++inIdx) {
    int intermediate = outInvariant(Const_In[inIdx]);
    out_reg += outDependent(intermediate, inIdx, i);
  }
  out[i] = out_reg;
}
...
static void s2g_gpu_gather(uint32_t *in, uint32_t *out, int len) {
  //@@ INSERT CODE HERE
  cudaMemcpyToSymbol(Const_In, in, len * sizeof(uint32_t));
  s2g_gpu_gather_kernel<<<ceil(len/(float)TILE_WIDTH),TILE_WIDTH>>>(in, out, len);
}
