...
//                  increasing order of position
// bin_ptrs       -- length (NUM_BINS + 1) array of indices into in_val_sorted
...
__global__ void gpu_normal_kernel(float *in_val, float *in_pos, float *out,
  ...
  if (idx < grid_size){
    for (int i = 0; i < num_in; ++i){
      const float d = in_pos[i] - idx;
      const float v = in_val[i];
      ans += v * v / (d * d);
    }
    out[idx] = ans;
  }
...
__global__ void gpu_cutoff_binned_kernel(int *bin_ptrs,
  ...
  const float cutoff = sqrtf(cutoff2);
  float ans = 0;

  const float right = (bx + 1) * bs - 1 + cutoff;
  const float left = bx * bs - cutoff;
  const int max_binIdx = (right < grid_size) ? (int)(right / grid_size * NUM_BINS) : NUM_BINS-1;
  const int min_binIdx = (left >= 0) ? (int)(left / grid_size * NUM_BINS) : 0;
  ...
  for (int i = bin_ptrs[min_binIdx]; i < max_inIdx; i += bs){
    // Load
    if (tx + i < max_inIdx){
    ...
    // Compute
    const int start = max(bin_ptrs[min_thread_binIdx] - i, 0);
    const int end = max(0, min(max_thread_inIdx - i, BLOCK_SIZE));
    for (int j = start; j < end; ++j){
      const float d2 = (idx - shared_pos[j]) * (idx - shared_pos[j]);
      if (d2 < cutoff2)
        ans += shared_val[j] * shared_val[j] / d2;
    }
    __syncthreads();
  }
  if (idx < grid_size)
...
__global__ void scan(int *bin_counts, int *bin_ptrs, int num_in) {

  //@@ INSERT CODE HERE
  #define BLOCK_SIZE 512
  __shared__ float SM[BLOCK_SIZE * 2];
  int bx = blockIdx.x, tx = threadIdx.x, x1 = bx * BLOCK_SIZE * 2 + tx, x2 = x1 + BLOCK_SIZE;

  if (x1 < num_in) SM[tx] = bin_counts[x1];
  else SM[tx] = 0;

  if (x2 < num_in)
    SM[tx + BLOCK_SIZE] = bin_counts[x2];
  else SM[tx + BLOCK_SIZE] = 0;
  ...
  if (x1 < num_in) bin_ptrs[x1] = SM[tx];
  if (x2 < num_in) bin_ptrs[x2] = SM[tx + BLOCK_SIZE];
  if(tx == 0)
    bin_ptrs[0] = 0;
  #undef BLOCK_SIZE
}
...
static void gpu_preprocess(float *in_val, float *in_pos,
  ...
  scan<<<1, numThreadsPerBlock>>>(bin_counts, bin_ptrs, num_in);
